...
class Whu(torch.utils.data.Dataset):
    """WHU-RS19 dataset."""
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for WHU-RS19".format(mode)
        logger.info("Constructing WHU-RS19 {}...".format(mode))
    ...
    def _construct_imdb(self):
        ...
        # Images are stored per class in subdirs
        # imagenet：(format: n<number>)
        split_files = pathmgr.ls(split_path)
        # self._class_ids = sorted(f for f in split_files if re.match(r"^n[0-9]+$", f))
        # self._class_ids = sorted(f for f in split_files)  # sorted：返回新列表
        self._class_ids = split_files  # 所有的类名
        # Map ImageNet class ids to contiguous ids
        self._class_id_cont_id = {v: i for i, v in enumerate(self._class_ids)}  # i：数字；v：类名
